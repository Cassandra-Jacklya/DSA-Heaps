...
public class FileIO {
	private DSAHeap heapArr;
	
	/********************************************************************
	** Submodule name: readFile
	** Import: fileName (String)
	** Export: hash (DSAHashTable)
	** Purpose: to read in the edges from the file and add into the heap array
	**********************************************************************/
	public DSAHeap readFile(String fileName) {
	    String word = "";
		heapArr = new DSAHeap(countLines(fileName));
	    ...
	    try {
			...
			fileStream.close();
			System.out.println("File '" + fileName + "' has been read and saved to a heap array");
		}
	    catch(IOException e) {
			if (fileStream != null) {
				try {
	...
	** Import: line (String)
	** Export: word (String)
	** Purpose: to separate certain parts of the string to add into the heap array
	****************************************************************************/
	...
	private String processLine(String line) {
		...
		heapArr.add(id, word);		//splitLine[0] represents the key (student ID)
		return word;
	}
	...
	** Export: none
	** Purpose: writes the key + value of the heap array into a csv file
	**********************************************************************/
	public void writeFile(String fileName, String writeArray) {
	    ...
	    try {
			...
			printLine.close();
			System.out.println("Data in heap has been written to file '" + fileName + "'");
	    }
	    catch(IOException e) {
			if (fileStream != null) {
				try {
	...
	** Purpose: converts the string of the key + value of heap array to csv file format
	****************************************************************************/
